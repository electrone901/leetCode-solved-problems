 ...
 ********************************************************/
/*
in: 9 => 11: if(root.right) go right & left til last
in: 12 => 14
in: 5 => 9 if(!root.right && !root.left)  go up & return 
in: 11 => 12
in: 14 => 20 check parents & return something bigger than node 
in: 20 => 25 
*/
...
in: 20 => 25 
  */
  ...
  if(inputNode.right) {
    let cur = inputNode.right;
    
    //case 1 if node.right doesn't have left then thats the answer
    if(!cur.left) return cur;
    
    //case 2 if node.right does have left then move til last left & return it
    while(cur.left) {
      cur = cur.left;
    }
    return cur;
  ...
  //case 3 node is a tree leaf then call its parent then loop until inputNode.key < par.key then return its parent node
  else {
    let par = inputNode.parent;
    
    while(par) {
      if(inputNode.key < par.key) return par;
      par = par.parent 
    }
  }
