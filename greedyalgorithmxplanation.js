/*
Greedy Algorithms
For some programming problems you may find that there is a 'greedy' way to solve it. A greedy approach 
means that at every step we take the best option for us with the current circumstances.

For example, consider the following situation: There is a pile of coins containing: 2 quarters, 3 dimes, and 
10 pennies We are allowed to take 3 coins from the pile. How do we maximize our profit? The greedy approach
would be to always take the coin with the highest value. In this case, the first coin we take would be a 
quarter. We would then take another quarter. Our third coin would be a dime since that is now the highest 
value coin.

This greedy approach can also be applied to a handful of common problems. When appropriate, the greedy 
approach is a great way to solve a problem. However, the difficulty lies in recognizing whether a problem 
can be correctly solved greedily.
